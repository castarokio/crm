...
import { Phone, Folder, Shield, LogOut, Loader2, Award, CheckCircle, BarChart3 } from 'lucide-react';
...
import { getTargetInventoryCounts } from '@/app/actions/leads';
...
export default function Dashboard({
  ...
  const [targets, setTargets] = useState<{
    ...
    calls_today: number;
  }>({ daily_call_target: 80, weekly_appointment_target: 15, calls_today: 0 });
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 flex flex-col relative overflow-hidden select-none">
      ...
      <div className="flex-1 flex flex-col md:flex-row h-screen overflow-hidden z-10">
        ...
        <main className="flex-1 overflow-hidden p-6 flex flex-col gap-6">
          <div className="flex-1 overflow-y-auto pr-1">
            {activeTab === 'dialer' && (
              <DialerTab callerName={callerName} />
            )}
            
            {activeTab === 'pipeline' && (
              <PipelineTab callerName={callerName} />
            )}
            
            {activeTab === 'directory' && (
              <DirectoryTab callerName={callerName} callerRole={callerRole} />
            )}
